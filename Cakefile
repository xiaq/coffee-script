...
code_for = (file, executable = false, show_load = true) ->
  ...
  cshtml  = "<pre class='tomorrow'><code>#{cshtml}</code></pre>"
  jshtml  = "<pre class='tomorrow'><code>#{jshtml}</code></pre>"
  jshtml = jshtml.split(/\n{2,}/).join('\n\n')
...
hand_iced = (file) ->
  @counter += 1
  return '' unless fs.existsSync("documentation/coffee/#{file}_hi.coffee")
  ics = fs.readFileSync("documentation/coffee/#{file}.coffee", 'utf-8')
  hcs = fs.readFileSync("documentation/coffee/#{file}_hi.coffee", 'utf-8')
  render = (x) ->
    "<pre class='tomorrow'><code>#{hljs.highlight('coffeescript', x).value}</code></pre>"
  hcshtml = render hcs
  icshtml = render ics
  name   = "example#{@counter}"
  script = "<script>window.#{name} = #{JSON.stringify(ics)}</script>"
  _import = "<div class='minibutton load' onclick='javascript: loadConsole(#{name});'>load</div>"
  "<div class='code'>#{icshtml}#{hcshtml}#{script}#{_import}<br class='clear' /></div>"
